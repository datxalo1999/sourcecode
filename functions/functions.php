...
if(mysqli_connect_errno()){//tra ve loi cu phap cua lan ket noi gan nhat
...
function cart(){
  ...
  if(isset($_GET['add_cart'])){// kiem tra bien nay da khoi tao chua
	  
	  $product_id = $_GET['add_cart'];
	  
	  $ip = get_ip();
	  
	  $run_check_pro = mysqli_query($con,"select * from cart where product_id='$product_id'");//truy van csdl
	
	  if(mysqli_num_rows($run_check_pro) > 0){//tra   so hang trong ket qua truyen vao
	     echo "";
	  }else{
	    
		$fetch_pro = mysqli_query($con, "select * from products where product_id='$product_id'");//truy van csdl
		
		$fetch_pro = mysqli_fetch_array($fetch_pro);//tra ve 1 dong ket qua cua truy van trong mysql duoi dang mảng
		...
		while($row_pro = mysqli_fetch_array($run_pro)){
		  ...
		  echo "
		      <div id='single_product'>
				...
				<p><b> Giá: $ $pro_price </b></p>
				
				<a href='details.php?pro_id=$pro_id'>Chi tiết</a>
				
				<a href='index.php?add_cart=$pro_id'>
				  <button style='float:right'>Giỏ hàng</button>
				</a>
			  </div>
		  ";
		}
